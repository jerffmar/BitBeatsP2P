...
const ArtistPage: React.FC<Props> = ({ swarmTracks, onPlay, currentTrackId }) => {
  const { id } = useParams<{ id: string }>();
  const tracks = swarmTracks.filter((track) => (track as any).artistId === id);
  ...
  return (
    <section className="max-w-4xl mx-auto p-6 md:p-10 space-y-6">
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 flex flex-col gap-2">
        <p className="text-xs uppercase tracking-[0.4em] text-gray-500">Artist</p>
      ...
      {tracks.length === 0 ? (
        <div className="text-center text-gray-500 border border-white/10 rounded-2xl py-16">No torrents for this artist yet.</div>
      ) : (
